...
- 文件夹和媒体提供复选框，支持整组全选/取消后再排除单个文件；顶部只保留“开始识别”和“开始翻译”，按任务树顺序处理所有已勾选文件。
- 右键媒体可单独转文字、翻译或查看详情；双击会切换媒体，并在中央紧凑详情区显示处理阶段和进度。
- SoVITS 改配音仅在任务菜单中标记为“暂未实现”，不注册实现，也不引入相关依赖。
- GUI 不显示项目文件概念。识别完成立即生成原文 SRT；勾选已有原文 SRT 的任务并点击“开始翻译”后生成中文 SRT。
- 恢复游标、缓存和修订只保存在 `data/state/*.sqlite3` 内部状态中，不需要用户打开或管理。
- 新媒体状态默认不启用翻译；只有用户点击“开始翻译”后才创建 Provider、读取 API Key 并发起请求。
...
- GUI 将 SRT 自动生成到绿色字幕目录，文件名与原媒体相同；CLI 仍支持 SRT、VTT、ASS、TXT 和 JSON。
...
主工具栏只保留“开始识别”和“开始翻译”。添加媒体、导入文件夹和打开字幕文件夹位于“文件”；批量操作、暂停和字幕检查位于“处理”；模型、GPU 与翻译服务位于“设置”。GUI 不提供手动导出按钮，识别与翻译成功后自动写入同名 SRT。
...
使用远程翻译服务时，字幕、提示词、标题、背景、人物关系和术语可能发送给界面显示的服务商。只有用户勾选文件并点击“开始翻译”时才会发送这些数据；识别不会自动调用翻译服务。离线模式下，程序禁止远程 HTTP 请求；已下载的本地模型仍可使用。
...
媒体加入任务树后不会自动处理。勾选文件并点击“开始识别”后固定生成原文 SRT，不会自动调用翻译 API；再次勾选已有原文 SRT 的媒体或直接拖入的 SRT，点击“开始翻译”后生成同名中文 SRT。任务异常时可通过“处理 → 强制暂停”终止当前模型/API 请求并清空尚未开始的队列。
