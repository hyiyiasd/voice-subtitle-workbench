...
- 主工具栏最终精简为“开始识别”和“开始翻译”两个按钮，均按任务树顺序处理所有已勾选文件。
...
- 任务树平时不显示进度；文件实际处理时在文件名前显示小型进度条，结束后自动收起。
- 翻译服务设置增加可编辑测试内容和真实接口测试，模型输出或完整 HTTP 错误通过弹窗显示。
- 增加强制暂停操作，可终止当前模型/API 请求、清空待处理队列，并允许运行中安全退出。
- GUI 改为明确的两步流程：媒体识别完成自动生成原文 SRT，再由用户选择原文 SRT 生成中文 SRT。
- 删除 GUI 手动导出、继续队列、翻译开关和重复翻译入口；原文与中文 SRT 均在对应任务完成后自动覆盖生成。
- 翻译上下文面板默认缩窄至约 240 像素，并在恢复默认布局时恢复该宽度。
- 移除 GUI 中 `.vstproj` 的新建、打开和拖放入口，恢复与缓存状态改存为内部 `data\state\*.sqlite3`。
- 完整重新识别按时间范围替换旧的未锁定字幕，修复重复识别后整套时间轴再次从零开始。
- DeepSeek 字幕翻译默认关闭思考模式并启用 JSON 输出，减少普通字幕批次等待时间。
...
- 更新 DeepSeek 官方 Base URL 和默认模型，自动迁移旧的 `deepseek-chat` / `deepseek-reasoner` 配置。
